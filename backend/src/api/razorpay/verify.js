...
const actualPlan = rows[0]?.pending_plan || planType;
    await query(
  ...
  [
   ...
   logger.info(
  `Payment verified & plan activated | plan=${actualPlan} | payment=${razorpay_payment_id} | user=${req.user.id}`,
);

   return res.json({
  success: true,
  plan: actualPlan,
  message: `${actualPlan.charAt(0).toUpperCase() + actualPlan.slice(1)} plan activated successfully!`,
});
  } catch (err) {
