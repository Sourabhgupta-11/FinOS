...
async function deleteAccount(req, res, next) {
  try {
    await query('DELETE FROM bank_accounts WHERE id = $1 AND user_id = $2', [req.params.id, req.user.id]);
...
async function initiateSetuConsent(req, res, next) {
  try {
    if (!process.env.SETU_CLIENT_ID) {
      return res.status(503).json({ error: 'Bank linking not configured. Use manual entry or CSV import.' });
    }

    const userWithPhone = { ...req.user, phone: req.body.phone };
    const consent = await setuService.createConsent(userWithPhone, req.body.fiTypes);

    res.json({ consentId: consent.id, redirectUrl: consent.redirectUrl, status: consent.status });
  } catch (err) { next(err); }
}
...
async function setuCallback(req, res, next) {
  try {
    ...
    if (status.status !== 'ACTIVE') {
      return res.json({ status: status.status, message: 'Consent not yet active' });
    }

    // Fetch FI data
    const fiData = process.env.NODE_ENV === 'development'
    ...
    const txs = setuService.parseSetuTransactions(fiData, accountId, req.user.id);
    let inserted = 0;
    for (const tx of txs) {
      try {
        await query(
          ...
          [tx.user_id, tx.bank_account_id, tx.type, tx.amount, tx.description,
           tx.merchant, tx.transaction_date, tx.source, tx.raw_data]
        );
        inserted++;
...
async function getTransactions(req, res, next) {
  try {
    const {
      from, to, type, categoryId, accountId,
    ...
    if (from)       { sql += ` AND t.transaction_date >= $${p++}`; params.push(from); }
    if (to)         { sql += ` AND t.transaction_date <= $${p++}`; params.push(to); }
    if (type)       { sql += ` AND t.type = $${p++}`; params.push(type); }
    if (categoryId) { sql += ` AND t.category_id = $${p++}`; params.push(categoryId); }
    if (accountId)  { sql += ` AND t.bank_account_id = $${p++}`; params.push(accountId); }
    if (search)     { sql += ` AND (t.description ILIKE $${p} OR t.merchant ILIKE $${p++})`; params.push(`%${search}%`); }
    ...
    const countRes = await query(
      `SELECT COUNT(*) FROM transactions WHERE user_id = $1`, [req.user.id]
    );
...
async function addTransaction(req, res, next) {
  try {
    const { bankAccountId, categoryId, type, amount, description, merchant,
            notes, tags, transactionDate, isRecurring, recurringInterval } = req.body;
    ...
    const { rows } = await query(
      `INSERT INTO transactions
       ...
       RETURNING *`,
      [req.user.id, bankAccountId || null, categoryId || null,
       type, amount, description, merchant || null,
       notes || null, tags || null, transactionDate,
...
async function updateTransaction(req, res, next) {
  try {
    const { categoryId, description, merchant, notes, tags, transactionDate } = req.body;
    const { rows } = await query(
      ...
      [categoryId, description, merchant, notes, tags, transactionDate, req.params.id, req.user.id]
    );
...
async function importCSV(req, res, next) {
  try {
    ...
    for (const row of records) {
      ...
      const desc = row.Description || row.Narration || row.narration || row.description || row.Particulars;
      const debit = parseFloat(row.Debit || row.debit || row.Withdrawal || row['Withdrawal Amount'] || 0);
      const credit = parseFloat(row.Credit || row.credit || row.Deposit || row['Deposit Amount'] || 0);
      const amount = debit > 0 ? debit : credit;
      const type = credit > 0 ? 'income' : 'expense';

      if (!date || !amount || !desc) { skipped++; continue; }
    ...
    const { rows } = await query(
      `INSERT INTO budgets (user_id, category_id, name, amount, period, start_date, alert_at_pct)
       ...
       ON CONFLICT (user_id, category_id) DO UPDATE SET
         ...
         alert_at_pct = EXCLUDED.alert_at_pct, updated_at = NOW()
       RETURNING *`,
      [req.user.id, categoryId || null, name, amount, period || 'monthly', alertAtPct || 80]
    );
    res.json(rows[0]);
  } catch (err) { next(err); }
}
...
async function deleteBudget(req, res, next) {
  try {
    await query('DELETE FROM budgets WHERE id = $1 AND user_id = $2', [req.params.id, req.user.id]);
...
async function getCategories(req, res, next) {
  try {
    const { rows } = await query(
      `SELECT * FROM categories WHERE user_id IS NULL OR user_id = $1 ORDER BY is_default DESC, name`,
      [req.user.id]
    );
...
module.exports = {
  ...
  initiateSetuConsent, setuCallback,
  getTransactions, addTransaction, updateTransaction, deleteTransaction,
  importCSV, getAnalytics,
  getBudgets, upsertBudget, deleteBudget, getCategories,
};
