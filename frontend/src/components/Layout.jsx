...
import {
  ...
  Target, Bell, Crown, Landmark, Sun, Moon, User, Zap, X, Receipt
} from 'lucide-react';
...
export default function Layout() {
  ...
  const planBadge = isPremium ? { label: 'Premium', cls: 'text-amber-500 dark:text-amber-400' }
    : isPro ? { label: 'Pro', cls: 'text-blue-500 dark:text-blue-400' }
          ...
          if (item.section) return (
            <div key={i} className="px-3 pt-4 pb-1.5">
              <div className="flex items-center gap-2">
                ...
                <span className="text-[10px] font-bold text-gray-300 dark:text-gray-600 uppercase tracking-widest">{item.section}</span>
                <div className="flex-1 h-px bg-gray-100 dark:bg-gray-800" />
              </div>
            </div>
          );
            ...
            <NavLink key={item.to} to={item.to} end={item.to === '/'}
              ...
              className={({ isActive }) =>
                `flex items-center gap-3 px-3 py-2.5 rounded-xl text-sm font-medium transition-all duration-150
                 ${isActive ? 'bg-blue-600 text-white shadow-sm shadow-blue-500/30'
                   : locked ? 'text-gray-300 dark:text-gray-600 cursor-default'
        ...
        <NavLink to="/subscription" onClick={() => setOpen(false)}
          className={({ isActive }) =>
            `flex items-center gap-2 px-3 py-2.5 rounded-xl text-sm font-semibold transition-all
             ${isActive
               ? (isPremium ? 'bg-amber-500 text-white' : isPro ? 'bg-blue-600 text-white' : 'bg-gray-200 dark:bg-gray-700 text-gray-900 dark:text-white')
               : isPremium ? 'bg-gradient-to-r from-amber-50 to-orange-50 dark:from-amber-900/20 dark:to-orange-900/20 text-amber-700 dark:text-amber-400 hover:from-amber-100 hover:to-orange-100'
               : isPro ? 'bg-blue-50 dark:bg-blue-900/20 text-blue-700 dark:text-blue-400 hover:bg-blue-100 dark:hover:bg-blue-900/30'
               : 'bg-gray-50 dark:bg-gray-800/60 text-gray-600 dark:text-gray-400 hover:bg-gray-100 dark:hover:bg-gray-800'}`
          }>
          {({ isActive }) => (<>
            {isPremium ? <Crown size={14} /> : isPro ? <Zap size={14} /> : <Receipt size={14} />}
            <span className="flex-1">My Subscription</span>
            {!isPremium && !isActive && (
              <span className={`text-[10px] font-bold px-1.5 py-0.5 rounded-md ${isPro ? 'bg-blue-100 dark:bg-blue-900/40 text-blue-600 dark:text-blue-400' : 'bg-gray-200 dark:bg-gray-700 text-gray-500'}`}>
                {isPro ? 'Pro' : 'Free'}
              </span>
            )}
          </>)}
        ...
        <NavLink to="/profile" onClick={() => setOpen(false)}
          ...
          <div className="w-8 h-8 rounded-full bg-gradient-to-br from-blue-500 to-indigo-600 flex items-center justify-center text-white text-xs font-bold flex-shrink-0 overflow-hidden">
            {localStorage.getItem('finos-avatar')
              ? <img src={localStorage.getItem('finos-avatar')} alt="avatar" className="w-full h-full object-cover" />
              : initials}
          </div>
    ...
    <div className="flex h-screen overflow-hidden bg-gray-50 dark:bg-gray-950">
      ...
      {open && (
        <div className="lg:hidden fixed inset-0 z-50 flex">
          ...
          <div className="relative z-10 h-full flex animate-[slideIn_0.2s_ease-out]"><SidebarInner /></div>
        </div>
      )}
