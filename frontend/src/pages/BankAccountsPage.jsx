...
import { Plus, Link2, Landmark, CreditCard, Wallet, Trash2, RefreshCw, Clock } from 'lucide-react';
...
const POPULAR_BANKS = [
  ...
  'AU Small Finance Bank','Ujjivan SFB','RBL Bank','IDBI Bank',
];
...
const EMPTY_FORM = {
  ...
  accountNumber: '', ifscCode: '', creditLimit: '', color: COLORS[0],
};
...
export default function BankAccountsPage() {
  ...
  useEffect(() => { if (!premLoading && isPremium) load(); else if (!premLoading) setLoading(false); }, [premLoading, isPremium]);
  ...
  async function handleAdd(e) {
    ...
    if (!['wallet','demat'].includes(form.accountType) && !form.ifscCode.trim()) {
    ...
    try {
      ...
      setForm(EMPTY_FORM);
      load();
    } catch (err) { setFormError(err.response?.data?.error || 'Failed to add account'); }
  }
  ...
  async function handleSetuLink() {
    ...
    try {
      ...
      if (res.data.redirectUrl) {
        ...
        alert('Complete bank linking in the popup. Balance and transactions will sync automatically.');
      } else {
  ...
  const totalBalance = accounts
    .filter(a => !['credit_card','loan'].includes(a.account_type) && a.balance != null)
  ...
  if (!isPremium) return <PremiumGate requiredPlan="premium" feature="Bank Account Linking" />;
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-gray-400 dark:text-gray-500 text-sm mt-0.5">Link accounts — balance fetched automatically</p>
        </div>
      ...
      {accounts.some(a => a.balance != null) && (
        <div className="grid grid-cols-3 gap-3">
          {[
            ...
            { label: 'Net liquid',       value: formatINR(totalBalance - totalDebt), color: (totalBalance - totalDebt) >= 0 ? 'text-emerald-600 dark:text-emerald-400' : 'text-red-600 dark:text-red-400' },
      ...
      <div className="card border-blue-100 dark:border-blue-900/40 bg-gradient-to-r from-blue-50 to-indigo-50 dark:from-blue-900/20 dark:to-indigo-900/20">
        ...
        <p className="text-xs text-blue-600 dark:text-blue-400 mb-3">
          Securely import balance and transactions automatically. Works with all major Indian banks. Balance refreshes every 30 days.
        </p>
        <div className="flex gap-2">
          <input className="input flex-1 text-sm" type="tel" placeholder="Mobile number linked to your bank"
            value={setuPhone} onChange={e => setSetuPhone(e.target.value.replace(/\D/g,''))} maxLength={10} />
      ...
      {showAdd && (
        <div className="card">
          ...
          <div className="bg-amber-50 dark:bg-amber-900/20 border border-amber-100 dark:border-amber-800/40 text-amber-700 dark:text-amber-300 text-xs px-3 py-2 rounded-xl mb-4 flex items-center gap-2">
            ...
            Balance will be automatically fetched after account is linked. No need to enter it manually.
          </div>
          <form onSubmit={handleAdd} className="space-y-4">
            ...
            <div className="flex gap-2">
              ...
              <button type="button" onClick={() => { setShowAdd(false); setFormError(''); setForm(EMPTY_FORM); }} className="btn-secondary text-sm py-2">Cancel</button>
            </div>
          </form>
        </div>
      )}
      ...
      {/* Account cards */}
      {loading ? (
        <div className="text-gray-400 dark:text-gray-600 animate-pulse text-sm">Loading accounts…</div>
      ) : accounts.length === 0 ? (
        <div className="card text-center py-14">
          ...
          <div className="text-gray-400 dark:text-gray-600 text-xs mt-1">Link via Setu AA or add manually above</div>
        </div>
      ) : (
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          {accounts.map(acc => {
            ...
            const utilPct = isCC && acc.credit_limit > 0 && hasBalance
              ? Math.round((acc.balance / acc.credit_limit) * 100) : null;
            const barColor = !utilPct ? '#10b981' : utilPct > 80 ? '#ef4444' : utilPct > 50 ? '#f59e0b' : '#10b981';
            const needsRefresh = acc.last_synced &&
              (Date.now() - new Date(acc.last_synced).getTime()) > 30 * 24 * 60 * 60 * 1000;
                ...
                <div className="pl-4">
                  <div className="flex items-start justify-between mb-3">
                    ...
                    <div className="flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                      <button onClick={() => handleSync(acc.id)} disabled={syncing === acc.id}
                        className="btn-ghost w-7 h-7 flex items-center justify-center" title="Refresh balance">
                  ...
                  {hasBalance ? (
                    ...
                    <div className="flex items-center gap-2">
                      <div className="text-sm text-gray-400 dark:text-gray-500">Balance pending sync</div>
                      <button onClick={() => handleSync(acc.id)} disabled={syncing === acc.id}
                  ...
                  {isCC && acc.credit_limit > 0 && hasBalance && (
                    <div className="mt-2">
                      <div className="flex justify-between text-xs mb-1">
                        <span className="text-gray-400 dark:text-gray-500">{formatINR(acc.balance)} of {formatINR(acc.credit_limit)}</span>
                        <span style={{ color: barColor }} className="font-semibold">{utilPct}% used</span>
                      </div>
                  ...
                  <div className="flex items-center gap-3 mt-2 text-xs text-gray-300 dark:text-gray-600">
                    {acc.last_synced && (
                      <span className={needsRefresh ? 'text-amber-500 dark:text-amber-400' : ''}>
                        {needsRefresh ? '⚠ Refresh needed · ' : ''}
                        Last sync: {new Date(acc.last_synced).toLocaleDateString('en-IN', { day:'numeric', month:'short' })}
                      </span>
                    )}
                  </div>
                </div>
