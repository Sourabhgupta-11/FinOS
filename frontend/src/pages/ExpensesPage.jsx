...
export default function ExpensesPage() {
  ...
  // Fetch AI category suggestions when description/merchant changes
  useEffect(() => {
    ...
    const fetchSuggestions = async () => {
      try {
        const res = await api.get("/bank/categories/suggestions", {
          params: {
  ...
  }, [form.description, form.merchant, form.type]);
  ...
  async function handleCSVImport() {
    ...
    try {
      ...
      alert(
        `✓ Imported ${res.data.inserted} transactions. ${res.data.skipped} skipped.`,
      );
      setCsvFile(null);
      load();
    } catch {
  ...
  function handleCSVExport() {
    ...
    a.href = url;
    a.download = `finos-transactions-${new Date().toISOString().split("T")[0]}.csv`;
    a.click();
  ...
  async function handleSyncAll() {
    ...
    try {
      ...
      const linkedAccs = (accRes.data.accounts || []).filter(
        (a) => a.setu_consent_id,
      );
      ...
      for (const acc of linkedAccs) {
        await api
          .post("/bank/setu/callback", {
            accountId: acc.id,
            consentId: acc.setu_consent_id,
          })
          .catch(() => {});
      }
      load();
  ...
  if (!isPro)
    return <PremiumGate requiredPlan="pro" feature="Expense Manager" />;
  ...
  return (
    <div className="space-y-5">
        ...
        <div className="flex gap-2 flex-wrap">
          ...
          {csvFile && (
            ...
            >
              {csvLoading
                ? "Importing…"
                : `Import "${csvFile.name.slice(0, 15)}…"`}
            </button>
          )}
          <button
        ...
        {[
          {
            ...
            value: formatINR(totals.month_income || 0),
            color: "text-emerald-600 dark:text-emerald-400",
          },
          {
            label: "Spent this month",
            value: formatINR(totals.month_expense || 0),
            color: "text-red-600 dark:text-red-400",
          },
          {
            label: "Net this month",
            value: formatINR(monthNet),
            color:
              monthNet >= 0
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-5">
        ...
        {/* Transactions */}
        <div
          className={`${byCategory.length > 0 ? "lg:col-span-2" : "lg:col-span-3"} space-y-3`}
        >
          {/* Add form */}
          {showAdd && (
            <div className="card">
          ...
          <div className="card divide-y divide-gray-50 dark:divide-gray-800/60">
            {loading ? (
              <div className="text-center py-10 text-gray-400 dark:text-gray-600 text-sm animate-pulse">
                Loading…
              </div>
            ) : txs.length === 0 ? (
              <div className="text-center py-12">
                ...
                <div className="text-gray-300 dark:text-gray-700 text-xs mt-1">
                  Add manually, import CSV, or sync your bank
                </div>
              </div>
            ) : (
              txs.map((tx) => (
                ...
                >
                  ...
                  <div
                    className={`text-sm font-bold tabular-nums flex-shrink-0 ${tx.type === "income" ? "text-emerald-600 dark:text-emerald-400" : "text-red-600 dark:text-red-400"}`}
                  >
                    {tx.type === "income" ? "+" : "-"}
                    {formatINR(tx.amount)}
                  </div>
