...
import { Crown, Zap, Check, X, Sparkles, AlertTriangle, Calendar, RefreshCw, Settings } from 'lucide-react';

const PLANS = [
  {
    key: 'free', name: 'Free', priceLabel: '₹0', period: 'forever',
    icon: null, color: 'text-gray-500 dark:text-gray-400', aiMsg: '3 msg/day',
    features: [
  ...
  {
    key: 'pro', name: 'Pro', priceLabel: '₹99', period: '/month',
    icon: Zap, color: 'text-blue-600 dark:text-blue-400', aiMsg: '50 msg/day',
  ...
  {
    key: 'premium', name: 'Premium', priceLabel: '₹199', period: '/month',
    icon: Crown, color: 'text-amber-500 dark:text-amber-400', aiMsg: 'Unlimited',
...
export default function SubscriptionPage() {
  ...
  const upgrade = async (planKey) => {
    ...
    try {
      ...
      if (res.data.demo) {
        setSuccess(`✓ ${planKey.charAt(0).toUpperCase() + planKey.slice(1)} plan activated (demo mode — configure Razorpay keys for live payments).`);
        setProcessing(null);
        setTimeout(() => window.location.reload(), 2000);
        return;
      }
      if (!window.Razorpay) { setError('Payment gateway not loaded. Please refresh.'); setProcessing(null); return; }
  ...
  const cancelSubscription = async () => {
    try {
      ...
      setSuccess('Subscription cancelled. You keep access until the end of your billing period.');
  ...
  if (subLoading) return <div className="text-gray-400 animate-pulse text-sm">Loading…</div>;
  ...
  return (
    <div className="space-y-6 max-w-3xl">
      ...
      <div className={`card ${
        currentPlan === 'premium' ? 'border-amber-200 dark:border-amber-800/60 bg-gradient-to-br from-amber-50 to-orange-50 dark:from-amber-900/20 dark:to-orange-900/20'
        : currentPlan === 'pro'  ? 'border-blue-200 dark:border-blue-800/60 bg-gradient-to-br from-blue-50 to-indigo-50 dark:from-blue-900/20 dark:to-indigo-900/20'
        : ''
      }`}>
        <div className="flex items-start justify-between flex-wrap gap-3 mb-4">
          <div className="flex items-center gap-3">
            {PlanIcon
              ? <div className={`w-10 h-10 rounded-xl flex items-center justify-center ${currentPlan === 'premium' ? 'bg-amber-100 dark:bg-amber-900/40' : 'bg-blue-100 dark:bg-blue-900/40'}`}>
            ...
            <div>
              ...
              <div className="text-xs text-gray-500 dark:text-gray-400">
                {planInfo?.priceLabel}{planInfo?.period !== 'forever' ? planInfo?.period : ''} · {planInfo?.aiMsg} AI messages
              </div>
            </div>
          </div>
        </div>
            ...
            {periodStart && (
              <div className="flex items-center gap-2.5 p-3 bg-white/70 dark:bg-gray-800/70 rounded-xl">
                ...
                <div>
                  ...
                  <div className="text-sm font-semibold text-gray-900 dark:text-white">{periodStart}</div>
                </div>
              </div>
            )}
            {periodEnd && (
              <div className="flex items-center gap-2.5 p-3 bg-white/70 dark:bg-gray-800/70 rounded-xl">
                ...
                <div>
                  ...
                  <div className="text-sm font-semibold text-gray-900 dark:text-white">{periodEnd}</div>
                </div>
              </div>
            )}
          </div>
        ...
        <div className="flex flex-wrap gap-2">
          ...
          {isCancelling && (
            <div className="text-xs text-amber-700 dark:text-amber-300 bg-amber-50 dark:bg-amber-900/30 px-3 py-2 rounded-xl border border-amber-200 dark:border-amber-800/40">
              Your subscription cancels on {periodEnd}. After that you'll move to Free.
            </div>
          )}
          {currentPlan === 'free' && (
            <div className="text-xs text-gray-500 dark:text-gray-400">You're on the Free plan. Upgrade below to unlock more features.</div>
          )}
        </div>
      </div>
        ...
        <div className="card border-red-200 dark:border-red-800/60 bg-red-50 dark:bg-red-900/20">
          <div className="flex items-start gap-3">
            ...
            <div className="flex-1">
              <div className="font-semibold text-red-900 dark:text-red-200 text-sm mb-1">Are you sure you want to cancel?</div>
              <div className="text-xs text-red-700 dark:text-red-300 leading-relaxed mb-3">
                You'll keep all {currentPlan.charAt(0).toUpperCase() + currentPlan.slice(1)} features until <strong>{periodEnd}</strong>.
                After that, your account moves to the Free plan and premium features will be locked.
              </div>
              <div className="flex gap-2">
                ...
                <button onClick={() => setShowCancelConfirm(false)} className="btn-secondary text-sm py-2">
                  Keep subscription
                </button>
              </div>
            </div>
      ...
      <div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          {PLANS.map((p) => {
            ...
            const canUpgrade = PLAN_ORDER[p.key] > PLAN_ORDER[currentPlan] && !isCancelling;
            const Icon = p.icon;
            return (
              <div key={p.key} className={`card relative flex flex-col ${isCurrent ? (p.borderClass || 'border-2 border-gray-300 dark:border-gray-600') : (p.borderClass || '')}`}>
                {p.badge && (
                ...
                <div className="mb-3 pt-1">
                  <div className="flex items-center gap-1.5 mb-1.5">
                    ...
                    {isCurrent && <span className="text-xs text-gray-400 dark:text-gray-500 ml-1">(current)</span>}
                  </div>
                  ...
                  {p.features.map((f, i) => (
                    <div key={i} className="flex items-center gap-2 text-xs">
                      {f.ok
                        ...
                        : <X size={11} className="text-gray-200 dark:text-gray-700 flex-shrink-0" />}
                ...
                {isCurrent ? (
                  ...
                  <button onClick={() => upgrade(p.key)} disabled={!!processing}
                    ...
                    {Icon && <Icon size={13} />}
                    {processing === p.key ? 'Opening payment…' : `Upgrade to ${p.name}`}
                  </button>
                ) : (
