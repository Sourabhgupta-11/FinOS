...
export default function BudgetsPage() {
  ...
  useEffect(() => { if (!premLoading && isPro) load(); else if (!premLoading) setLoading(false); }, [premLoading, isPro]);
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-gray-400 dark:text-gray-500 text-sm mt-0.5">Set monthly spending limits and get alerts</p>
        </div>
      ...
      <div className="grid grid-cols-3 gap-3">
        {[
          { label: 'Total budgeted', value: formatINR(totalBudget), color: 'text-gray-900 dark:text-white' },
          { label: 'Total spent',    value: formatINR(totalSpent),  color: totalSpent > totalBudget ? 'text-red-600 dark:text-red-400' : 'text-gray-900 dark:text-white' },
          { label: 'Over budget',    value: `${overBudget} item${overBudget !== 1 ? 's' : ''}`, color: overBudget > 0 ? 'text-red-600 dark:text-red-400' : 'text-emerald-600 dark:text-emerald-400' },
        ...
        <div className="card">
          ...
          <form onSubmit={handleAdd} className="space-y-3">
            <div className="grid grid-cols-2 gap-3">
              <div><label className="label">Budget name</label>
                <input className="input" placeholder="Dining out" value={form.name} onChange={e => set('name', e.target.value)} required /></div>
              ...
              <div><label className="label">Monthly limit (₹)</label>
                <input className="input" type="number" min="1" placeholder="3000" value={form.amount} onChange={e => set('amount', e.target.value)} required /></div>
              <div><label className="label">Alert threshold</label>
                <select className="input" value={form.alertAtPct} onChange={e => set('alertAtPct', parseInt(e.target.value))}>
                  {[50,60,70,75,80,90,100].map(p => <option key={p} value={p}>{p}% used</option>)}
                </select></div>
            </div>
      ...
      {loading ? (
        <div className="text-gray-400 dark:text-gray-600 animate-pulse text-sm">Loading budgets…</div>
      ) : budgets.length === 0 ? (
        <div className="card text-center py-14">
          <Target size={36} className="text-gray-200 dark:text-gray-700 mx-auto mb-3" />
          <div className="text-gray-500 dark:text-gray-400 text-sm font-medium">No budgets yet</div>
          <div className="text-gray-400 dark:text-gray-600 text-xs mt-1">Create a budget to control your spending</div>
        </div>
      ) : (
              ...
              <div key={b.id} className="card-hover group">
                <div className="flex items-start justify-between mb-3">
                  <div>
                    ...
                    <div className="text-xs text-gray-400 dark:text-gray-500 mt-0.5">{b.category_name || 'All expenses'} · {b.period}</div>
                  </div>
                  <div className="flex items-center gap-2">
                    {pct >= 100 ? <AlertTriangle size={14} className="text-red-500" /> : pct >= b.alert_at_pct ? <AlertTriangle size={14} className="text-amber-500" /> : <CheckCircle size={14} className="text-emerald-500" />}
                    <button onClick={() => handleDelete(b.id)} className="opacity-0 group-hover:opacity-100 p-1 text-gray-300 hover:text-red-500 transition-all">
                      <Trash2 size={13} />
                    </button>
                  </div>
                </div>
                ...
                <div className="flex justify-between text-xs mb-2">
                  <span className="text-gray-600 dark:text-gray-400">Spent <strong className="text-gray-900 dark:text-white">{formatINR(b.spent || 0)}</strong></span>
                  <span className="text-gray-400 dark:text-gray-500">of {formatINR(b.amount)}</span>
                </div>
                ...
                <div className="flex justify-between items-center">
                  ...
                  <span className={`text-xs ${remaining >= 0 ? 'text-gray-400 dark:text-gray-500' : 'text-red-600 dark:text-red-400 font-semibold'}`}>
