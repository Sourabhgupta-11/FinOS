...
export default function TaxCalculatorPage() {
    ...
    try {
      const payload = Object.fromEntries(
        Object.entries(form).map(([k, v]) => [
          k,
          typeof v === "boolean" ? v : v === "" ? 0 : parseFloat(v) || v,
        ]),
      );
  ...
  if (!isPremium)
    return <PremiumGate requiredPlan="premium" feature="Tax Calculator" />;

  const chartData = result
    ? [
        {
          name: "Old regime",
          tax: result.oldRegime.tax,
          fill: result.recommended === "old" ? "#10b981" : "#ef4444",
        },
        {
          name: "New regime",
          tax: result.newRegime.tax,
          fill: result.recommended === "new" ? "#10b981" : "#3b82f6",
        },
      ]
    : [];
    ...
    <div className="space-y-6">
      <div>
        ...
        <p className="text-gray-400 dark:text-gray-500 text-sm mt-0.5">
          FY 2024-25 · Old vs New regime with 87A rebate, surcharge & cess
        </p>
      </div>
      ...
      {result && (
        <div className="space-y-4 page-enter">
          {/* Recommendation */}
          <div
            className={`card flex items-center gap-4 ${result.recommended === "new" ? "border-blue-200 dark:border-blue-800/60 bg-blue-50 dark:bg-blue-900/20" : "border-emerald-200 dark:border-emerald-800/60 bg-emerald-50 dark:bg-emerald-900/20"}`}
          >
            <CheckCircle
              size={24}
              className={
            ...
            <div>
              ...
              <div className="text-sm text-gray-600 dark:text-gray-400">
                Save {formatINR(result.taxSaved)} by choosing the{" "}
                {result.recommended} tax regime
              </div>
            </div>
          </div>
          ...
          <div className="grid grid-cols-2 gap-4">
            {["oldRegime", "newRegime"].map((key) => {
              const r = result[key];
              const isRec =
                result.recommended === (key === "oldRegime" ? "old" : "new");
              return (
                <div
                  key={key}
                  className={`card ${isRec ? "border-2 border-blue-400 dark:border-blue-500" : ""}`}
                >
                  <div className="flex items-center justify-between mb-3">
                    ...
                    {isRec && (
                      <span className="badge-info text-xs">Recommended</span>
                    )}
                  </div>
                  <div className="text-2xl font-bold text-gray-900 dark:text-white tabular-nums mb-3">
                    {formatINR(r.tax)}
                  </div>
          ...
          <div className="card">
            ...
            <ResponsiveContainer width="100%" height={130}>
              <BarChart data={chartData} barSize={56}>
                ...
                <YAxis hide />
                <Tooltip
                  formatter={(v) => [formatINR(v), "Tax payable"]}
                  contentStyle={{
                    background: "white",
          ...
          <div className="card">
            ...
            <div className="space-y-2">
              {result.deductionBreakdown
                .filter((d) => d.oldRegime > 0 || d.newRegime > 0)
                .map((d) => (
                  <div
                    key={d.label}
                  ...
                  >
                    ...
                    <div className="w-24 text-right font-medium text-gray-700 dark:text-gray-300">
                      {formatINR(d.oldRegime)}
                    </div>
                    <div className="w-24 text-right font-medium text-blue-600 dark:text-blue-400">
                      {formatINR(d.newRegime)}
                    </div>
                  </div>
                ))}
          ...
          {result.tips?.length > 0 && (
            <div className="card">
              <div className="section-title flex items-center gap-2">
                <Lightbulb size={13} className="text-amber-500" /> Tax saving
                tips
              </div>
              <div className="space-y-2">
          ...
          <form onSubmit={handleSubmit} className="space-y-5">
            ...
            <div className="flex items-center gap-2.5">
              <input
                ...
                onChange={(e) => set("isMetroCity", e.target.checked)}
